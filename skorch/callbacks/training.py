...
class Checkpoint(Callback):
    ...
    def on_epoch_end(self, net, **kwargs):
        ...
        if do_checkpoint:
            self.save_model(net)
            self._sink("A checkpoint was triggered in epoch {}.".format(len(net.history) + 1), net.verbose)

    def _f_kwargs(self):
        return {key: getattr(self, key) for key in dir(self) if key.startswith("f_") and (key != "f_history_")}
    ...
    def save_model(self, net):
        ...
        """
        kwargs_module, kwargs_other = _check_f_arguments(self.__class__.__name__, **self._f_kwargs())
    ...
    def _save_params(self, f, net, f_name, log_name):
        try:
            ...
            self._sink("Unable to save {} to {}, {}: {}".format(log_name, f, type(e).__name__, e), net.verbose)
...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, net, **kwargs):
        ...
        if self.misses_ == self.patience:
            if net.verbose:
                self._sink(
                    "Stopping since {} has not improved in the last " "{} epochs.".format(self.monitor, self.patience),
                    verbose=net.verbose,
                )
    ...
    def on_train_end(self, net, **kwargs):
        if (
            ...
            self._sink("Restoring best model from epoch {}.".format(self.best_epoch_), verbose=net.verbose)
...
class ParamMapper(Callback):
    ...
    def initialize(self):
        ...
        if isinstance(self.at, int):
            if self.at <= 0:
                raise ValueError(
                    "Invalid value for `at` (at={}). The first possible " "epoch number is 1.".format(self.at)
                )
    ...
    def filter_parameters(self, patterns, params):
        pattern_fns = (pattern if callable(pattern) else partial(fnmatch, pat=pattern) for pattern in patterns)
        ...
        if not self.did_load_:
            ...
            with suppress(FileNotFoundError):
                if isinstance(self.checkpoint, TrainEndCheckpoint):
                    net.load_params(checkpoint=self.checkpoint.checkpoint_, use_safetensors=self.use_safetensors)
                else:
                    net.load_params(checkpoint=self.checkpoint, use_safetensors=self.use_safetensors)
