...
def trim_for_prediction(net, trim_criterion=True, trim_history=True):
    ...
    ``net.predict`` or ``net.predict_proba``) but no longer for training (e.g.
    ``net.fit(X, y)`` will raise an exception).
    ...
    operation mutates the passed net. If you need the unmodified net, create a
    deepcopy before trimming:
    ...
    if isinstance(net.callbacks, list):
        net.callbacks.clear()
