...
class TestLRCallbacks:
    ...
    def test_lr_callback_init_policies(
        ...
        assert any(
            list(
                map(
                    lambda x: isinstance(getattr(x[1], 'lr_scheduler_', None), instance),
                    net.callbacks_,
                )
            )
        )
    ...
    def test_lr_scheduler_record_epoch_step(
        self, classifier_module, classifier_data, policy, kwargs
    ):
        epochs = 3
        scheduler = LRScheduler(policy, **kwargs)
        lrs = scheduler.simulate(epochs, initial_lr=123.0)
        net = NeuralNetClassifier(
            classifier_module, max_epochs=epochs, lr=123.0, callbacks=[('scheduler', scheduler)]
        )
    ...
    def test_reduce_lr_record_batch_step(self, classifier_module, classifier_data):
        ...
        net = NeuralNetClassifier(
            ...
            callbacks=[
                (
                    'scheduler',
                    LRScheduler(ReduceLROnPlateau, monitor='train_loss', step_every='batch'),
                ),
            ],
        )
        net.fit(*classifier_data)
...
class TestWarmRestartLR:
    def assert_lr_correct(
        self, optimizer, targets, epochs, min_lr, max_lr, base_period, period_mult
    ):
    ...
    def test_single_period(self, init_optimizer):
        ...
        self.assert_lr_correct(
            optimizer, targets, epochs, min_lr, max_lr, base_period, period_mult
        )
    ...
    def test_restarts_with_multiple_groups(self, classifier_module):
        ...
        for min_lr, max_lr in zip(min_lr_group, max_lr_group):
            targets.append(
                self._multi_period_targets(epochs, min_lr, max_lr, base_period, period_mult)
            )
        self.assert_lr_correct(
            optimizer, targets, epochs, min_lr_group, max_lr_group, base_period, period_mult
        )
