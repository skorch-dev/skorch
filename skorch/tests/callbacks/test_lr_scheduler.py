...
class TestLRCallbacks:
    ...
    def test_simulate_lrs_batch_step(self, policy):
        lr_sch = LRScheduler(policy, base_lr=1, max_lr=5, step_size_up=4, step_every="batch")
    ...
    def test_simulate_lrs_reduced_lr_on_plateau_scalar(self):
        ...
        lrs = lr_sch.simulate(steps=5, initial_lr=1, step_args=0.5)
        # O = OK epoch
    ...
    def test_simulate_lrs_reduced_lr_on_plateau_array(self):
        ...
        lrs = lr_sch.simulate(steps=5, initial_lr=1, step_args=metrics)
        # O = OK epoch
        # I = intertolerable epoch
        #
        # 1 2 3 4 5   epoch number
    ...
    def test_lr_callback_init_policies(
        ...
        net = NeuralNetClassifier(classifier_module, max_epochs=2, callbacks=[lr_policy])
        net.fit(X, y)
        assert any(list(map(lambda x: isinstance(getattr(x[1], "lr_scheduler_", None), instance), net.callbacks_)))
    ...
    def test_lr_callback_batch_steps_correctly(
        ...
        lr_policy = LRScheduler(policy, **kwargs)
        net = NeuralNetClassifier(
            classifier_module(), max_epochs=max_epochs, batch_size=batch_size, callbacks=[lr_policy]
        )
        net.fit(X, y)
    ...
    def test_lr_callback_batch_steps_correctly_fallback(
        ...
        # 80% of sample used for training by default
        total_iterations_partial_fit_run = 0.8 * total_iterations_per_epoch * max_epochs

        # called fit AND partial_fit
        total_iterations = total_iterations_fit_run + total_iterations_partial_fit_run
    ...
    def test_lr_scheduler_set_params(self, classifier_module, classifier_data):
        scheduler = LRScheduler(CyclicLR, base_lr=123, max_lr=999, step_every="batch")
        net = NeuralNetClassifier(
            classifier_module,
            max_epochs=0,
    ...
    @pytest.mark.parametrize("policy,kwargs", [(StepLR, {"gamma": 0.9, "step_size": 1})])
    def test_lr_scheduler_record_epoch_step(self, classifier_module, classifier_data, policy, kwargs):
        ...
        lrs = scheduler.simulate(epochs, initial_lr=123.0)
        net = NeuralNetClassifier(classifier_module, max_epochs=epochs, lr=123.0, callbacks=[("scheduler", scheduler)])
    ...
    def test_lr_scheduler_record_batch_step(self, classifier_module, classifier_data):
        ...
        scheduler = LRScheduler(CyclicLR, base_lr=1, max_lr=5, step_size_up=4, step_every="batch")
        net = NeuralNetClassifier(
            classifier_module, max_epochs=1, lr=123.0, batch_size=batch_size, callbacks=[("scheduler", scheduler)]
        )
        net.fit(X, y)
...
class TestReduceLROnPlateau:
    def get_net_with_mock(self, classifier_data, classifier_module, monitor="train_loss"):
    ...
    def test_reduce_lr_monitor_with_string(self, monitor, classifier_data, classifier_module):
        # step should be called with the 2nd to last value from that
        # history entry
        net, mock_step = self.get_net_with_mock(classifier_data, classifier_module, monitor=monitor)
        score = mock_step.call_args_list[0][0][0]
        np.isclose(score, net.history[-2, monitor])

    def test_reduce_lr_monitor_with_callable(self, classifier_data, classifier_module):
        # step should always be called with the return value from the
        # callable, 55
        _, mock_step = self.get_net_with_mock(classifier_data, classifier_module, monitor=lambda x: 55)
        score = mock_step.call_args_list[0][0][0]
        assert score == 55

    @pytest.mark.parametrize("mode", ["min", "max"])
    def test_reduce_lr_monitor_passes_monitored_loss(self, classifier_data, classifier_module, mode):
        X, y = classifier_data
        net = NeuralNetClassifier(
            classifier_module,
            callbacks=[
                ("scheduler", LRScheduler(ReduceLROnPlateau, monitor="valid_loss", mode=mode)),
            ],
            max_epochs=1,
        )
        net.fit(X, y)
    ...
    def test_reduce_lr_raise_error_when_key_does_not_exist(self, classifier_data, classifier_module):
        X, y = classifier_data
        net = NeuralNetClassifier(
            classifier_module,
            callbacks=[
                ("scheduler", LRScheduler(ReduceLROnPlateau, monitor="bad_key")),
            ],
            max_epochs=1,
        )
        msg = (
            "'bad_key' was not found in history. A Scoring "
    ...
    def test_reduce_lr_record_epoch_step(self, classifier_module, classifier_data):
        ...
        expected = np.unique([123.0, 12.3, 1.23])[-len(lrs_unique) :]
    ...
    def test_reduce_lr_record_batch_step(self, classifier_module, classifier_data):
        ...
        net = NeuralNetClassifier(
            ...
            lr=lr,
            callbacks=[
                ("scheduler", LRScheduler(ReduceLROnPlateau, monitor="train_loss", step_every="batch")),
            ],
        )
        net.fit(*classifier_data)
...
class TestWarmRestartLR:
    def assert_lr_correct(self, optimizer, targets, epochs, min_lr, max_lr, base_period, period_mult):
        """Test that learning rate was set correctly."""
        targets = [targets] if len(optimizer.param_groups) == 1 else targets
        scheduler = WarmRestartLR(optimizer, min_lr, max_lr, base_period, period_mult)
    ...
    def _multi_period_targets(self, epochs, min_lr, max_lr, base_period, period_mult):
        ...
        while remaining_epochs > 0:
            ...
            targets += self._single_period_targets(period_epochs, min_lr, max_lr, current_period)
    ...
    def test_single_period(self, init_optimizer):
        ...
        base_period = 3
        period_mult = 1
        targets = self._single_period_targets(epochs, min_lr, max_lr, base_period)
        self.assert_lr_correct(optimizer, targets, epochs, min_lr, max_lr, base_period, period_mult)
    ...
    def test_multi_period_with_restart(self, init_optimizer):
        ...
        base_period = 2
        period_mult = 2
        targets = self._multi_period_targets(epochs, min_lr, max_lr, base_period, period_mult)
        self.assert_lr_correct(optimizer, targets, epochs, min_lr, max_lr, base_period, period_mult)
    ...
    def test_restarts_with_multiple_groups(self, classifier_module):
        ...
        for min_lr, max_lr in zip(min_lr_group, max_lr_group):
            targets.append(self._multi_period_targets(epochs, min_lr, max_lr, base_period, period_mult))
        self.assert_lr_correct(optimizer, targets, epochs, min_lr_group, max_lr_group, base_period, period_mult)
