...
class TestCheckpoint:
    ...
    @pytest.mark.parametrize(
        "f_name, mode",
        [("f_params", "w"), ("f_optimizer", "w"), ("f_criterion", "w"), ("f_history", "w"), ("f_pickle", "wb")],
    )
    def test_init_with_dirname_and_file_like_object_error(self, checkpoint_cls, tmpdir, f_name, mode):
    ...
    def test_initialize_with_dirname_and_file_like_object_error(self, checkpoint_cls, tmpdir, f_name, mode):
        from skorch.exceptions import SkorchException
    ...
    def test_default_without_validation_raises_meaningful_error(self, net_cls, checkpoint_cls, data):
        net = net_cls(
            callbacks=[
                checkpoint_cls(),
            ],
    ...
    def test_save_all_targets_with_prefix_and_dirname(
        self, save_params_mock, pickle_dump_mock, net_cls, checkpoint_cls, data, tmpdir, use_safetensors
    ):
    ...
    def test_save_no_targets(self, save_params_mock, pickle_dump_mock, net_cls, checkpoint_cls, data):
        net = net_cls(
            callbacks=[
                checkpoint_cls(
                    monitor=None,
    ...
    def test_warnings_when_monitor_appears_in_history(self, net_cls, checkpoint_cls, save_params_mock, data):
    ...
    def test_save_custom_module(self, save_params_mock, module_cls, checkpoint_cls, data, use_safetensors):
        ...
        kwargs = {"use_safetensors": use_safetensors}
        save_params_mock.assert_has_calls([call(f_mymodule="mymodule.pt", **kwargs)] * len(net.history))
...
class TestEarlyStopping:
    ...
    def test_typical_use_case_nonstop(self, net_clf_cls, classifier_module, classifier_data, early_stopping_cls):
        patience = 5
        max_epochs = 8
    ...
    def test_weights_restore(self, net_clf_cls, classifier_module, classifier_data, early_stopping_cls):
        patience = 3
        max_epochs = 20
        seed = 1
        ...
        assert all(torch.equal(wi, wj) for wi, wj in zip(net2.module_.state_dict().values(), es_weights.values()))
    ...
    def test_stopping_big_absolute_threshold(
        ...
        patience = 5
        max_epochs = 8
        early_stopping_cb = early_stopping_cls(patience=patience, threshold_mode="abs", threshold=0.1)
    ...
    def test_wrong_threshold_mode(self, net_clf_cls, classifier_module, classifier_data, early_stopping_cls):
        patience = 5
        max_epochs = 8
...
class TestParamMapper:
    ...
    def test_freezing_is_effective(self, net_cls, classifier_module, classifier_data, freezer, mod_init, mod_kwargs):
        from skorch.utils import to_numpy
    ...
    def test_unfreezing_is_effective(self, net_cls, classifier_module, classifier_data, freezer, unfreezer):
        from skorch.utils import to_numpy
    ...
    def test_schedule_is_effective(self, net_cls, classifier_module, classifier_data, param_mapper):
...
class TestLoadInitState:
    ...
    def test_load_initial_state(self, checkpoint_cls, net_cls, loadinitstate_cls, data, tmpdir, use_safetensors):
        skorch_dir = tmpdir.mkdir("skorch")
