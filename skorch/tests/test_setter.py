...
class TestOptimizerSetter:
    ...
    def optimizer_dummy(self):
        ...
        optim.param_groups = [{"lr": 0.01, "momentum": 0.9}, {"lr": 0.02, "momentum": 0.9}]
        return optim
    ...
    def test_wrong_name_raises(self, setter, net_optim_dummy):
        ...
        assert e.value.args[0] == ('Invalid parameter "{param}" for optimizer "optimizer"'.format(param=param))
    ...
    def test_only_specific_param_group_updated(self, setter, net_optim_dummy, group, sub_param, value):
        ...
        assert all(old[sub_param] == new[sub_param] for old, new in zip(static_groups_pre, static_groups_new))
