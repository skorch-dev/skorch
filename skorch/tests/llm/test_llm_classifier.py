...
class TestZeroShotClassifier:
    ...
    def test_init_encoder_decoder_with_caching_raises(self, classifier_cls, model, tokenizer):
        msg = "Caching is not supported for encoder-decoder models, " "initialize the model with use_caching=False."
        clf = classifier_cls(model=model, tokenizer=tokenizer, use_caching=True)
        with pytest.raises(ValueError, match=msg):
            clf.fit(None, ["positive", "negative"])

    def test_init_encoder_decoder_with_caching_using_model_name_raises(self, classifier_cls, model, tokenizer):
        msg = "Caching is not supported for encoder-decoder models, " "initialize the model with use_caching=False."
    ...
    def test_predict_proba_not_normalized(self, model, tokenizer, classifier_cls, X):
        clf = classifier_cls(model=model, tokenizer=tokenizer, use_caching=False, probas_sum_to_1=False)
        clf.fit(None, ["negative", "positive"])
        y_proba = clf.predict_proba(X)
    ...
    def test_defective_prompt_missing_key_raises(self, model, tokenizer, classifier_cls, recwarn):
        ...
        msg = "The prompt may not be correct, it expects 2 " "placeholders: 'labels', 'text', missing keys: 'labels'"
        clf.fit(None, ["positive", "negative"])
    ...
    def test_defective_prompt_extra_key_raises(self, model, tokenizer, classifier_cls, recwarn):
        ...
        msg = "The prompt may not be correct, it expects 2 " "placeholders: 'labels', 'text', extra keys: 'examples'"
        clf.fit(None, ["positive", "negative"])
        assert str(recwarn.list[0].message) == msg
    ...
    def test_low_probability_error(self, classifier_cls, model, tokenizer, X):
        ...
        msg = r"The sum of all probabilities is \d\.\d+, " "which is below the minimum threshold of 0.99"
    ...
    def test_repr(self, classifier_cls, model, tokenizer):
        expected = (
            "ZeroShotClassifier(model='T5ForConditionalGeneration', " "tokenizer='T5TokenizerFast', use_caching=False)"
        )
        clf = classifier_cls(model=model, tokenizer=tokenizer, use_caching=False)
...
class TestFewShotClassifier:
    ...
    def test_fit_X_and_y_not_matching_raises(self, model, tokenizer, classifier_cls, X, y):
        ...
        # number of samples
        clf = classifier_cls(model=model, tokenizer=tokenizer, use_caching=False)
        msg = "X and y don't have the same number of samples, found 2 and 3 samples, " "respectively"
        with pytest.raises(ValueError, match=msg):
            clf.fit(X[:2], y[:3])
    ...
    def test_defective_prompt_missing_keys_raises(self, model, tokenizer, classifier_cls, X, y, recwarn):
    ...
    def test_defective_prompt_extra_keys_raises(self, model, tokenizer, classifier_cls, X, y, recwarn):
