...
class TestResolvePlaceholder:
    ...
    @pytest.fixture
    def dataset_uses_y_placeholder(self):
        from skorch.utils import dataset_uses_y_placeholder
        return dataset_uses_y_placeholder
    ...
    def test_dataset_uses_y_placeholder(
            self, dataset_cls, data, dataset_uses_y_placeholder):
        X, y = data
        ds = dataset_cls(X, y=None)
        assert dataset_uses_y_placeholder(ds)

    def test_dataset_uses_non_y_placeholder(
            self, dataset_cls, data, dataset_uses_y_placeholder):
        X, y = data
        ds = dataset_cls(X, y)
        assert not dataset_uses_y_placeholder(ds)

    def test_subset_dataset_uses_y_placeholder(
            self, dataset_cls, data, dataset_uses_y_placeholder,
            cv_split_cls):
        X, y = data
        ds = dataset_cls(X, y=None)
        ds_train, ds_valid = cv_split_cls(cv=2)(ds)
        assert dataset_uses_y_placeholder(ds_train)
        assert dataset_uses_y_placeholder(ds_valid)
    ...
    def test_subset_dataset_uses_non_y_placeholder(
        ...
        ds = dataset_cls(X, y)
        ds_train, ds_valid = cv_split_cls(cv=2)(ds)
        assert not dataset_uses_y_placeholder(ds_train)
        assert not dataset_uses_y_placeholder(ds_valid)
