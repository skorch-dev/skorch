...
class TestTrimForPrediction:
    ...
    def trim_for_prediction(self):
        from skorch.utils import trim_for_prediction

        return trim_for_prediction
    ...
    def test_trimmed_net_less_memory(self, net, trim_for_prediction):
    ...
    def test_trim_untrained_net_raises(self, net_untrained, trim_for_prediction):
        from skorch.exceptions import NotInitializedError

        with pytest.raises(NotInitializedError):
            trim_for_prediction(net_untrained)

    def test_try_trimmed_net_fit_raises(self, net, trim_for_prediction, classifier_data):
    ...
    def test_try_trimmed_net_partial_fit_raises(
            self, net, trim_for_prediction, classifier_data
    ):
        from skorch.exceptions import SkorchTrainingImpossibleError
    ...
    def test_inference_works(self, net, trim_for_prediction, classifier_data):
        # does not raise
        trim_for_prediction(net)
        X, _ = classifier_data
        net.predict(X)
    ...
    def test_trim_twice_works(self, net, trim_for_prediction):
        # does not raise
        trim_for_prediction(net)
        trim_for_prediction(net)

    def test_callbacks_trimmed(self, net, trim_for_prediction):
        trim_for_prediction(net)
        assert not net.callbacks
        assert not net.callbacks_

    def test_optimizer_trimmed(self, net, trim_for_prediction):
        trim_for_prediction(net)
        assert net.optimizer is None
        assert net.optimizer_ is None

    def test_criteria_trimmed(self, net_2_criteria, trim_for_prediction):
        trim_for_prediction(net_2_criteria)
        assert net_2_criteria.criterion is None
        assert net_2_criteria.criterion_ is None
        assert net_2_criteria.criterion2_ is None

    def test_criteria_left_untrimmed(self, net_2_criteria, trim_for_prediction):
        trim_for_prediction(net_2_criteria, trim_criterion=False)
        assert net_2_criteria.criterion is not None
        assert net_2_criteria.criterion_ is not None
        assert net_2_criteria.criterion2_ is not None

    def test_history_trimmed(self, net, trim_for_prediction):
        trim_for_prediction(net)
        assert not net.history

    def test_history_left_untrimmed(self, net, trim_for_prediction):
        trim_for_prediction(net, trim_history=False)
        assert net.history

    def test_module_training(self, net, trim_for_prediction):
        # pylint: disable=protected-access
        net._set_training(True)
        trim_for_prediction(net)
        assert net.module_.training is False

    def test_can_be_pickled(self, net, trim_for_prediction):
        pickle.dumps(net)
        trim_for_prediction(net)
        pickle.dumps(net)

    def test_can_be_copied(self, net, trim_for_prediction):
        deepcopy(net)
        trim_for_prediction(net)
        deepcopy(net)

    def test_can_be_cloned(self, net, trim_for_prediction):
        clone(net)
        trim_for_prediction(net)
        clone(net)
