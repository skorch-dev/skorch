...
class TestSliceDict:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (slice(None, None, -1), dict(f0=np.arange(4)[::-1], f1=np.arange(12).reshape(4, 3)[::-1])),
        ],
    )
...
class TestSliceDataset:
    ...
    def test_index_with_2d_array_raises(self, slds):
        ...
        msg = "SliceDataset only supports slicing with 1 " "dimensional arrays, got 2 dimensions instead."
        assert exc.value.args[0] == msg
...
class TestDataFrameTransformer:
    ...
    def test_invalid_dtype_raises(self, transformer_cls, df, data):
        ...
        expected = (
            "The following columns have dtypes that cannot be " "interpreted as numerical dtypes: invalid (object)"
        )
        assert msg == expected
