    ...
    def classes_(self):
        if self.classes is not None:
            if not len(self.classes):
                raise AttributeError("{} has no attribute 'classes_'".format(self.__class__.__name__))
...
class NeuralNetBinaryClassifier(ClassifierMixin, NeuralNet):
    ...
    def infer(self, x, **fit_params):
        ...
        if (y_infer.dim() > 2) or ((y_infer.dim() == 2) and (y_infer.shape[1] != 1)):
            raise ValueError(
                "Expected module output to have shape (n,) or " "(n, 1), got {} instead".format(tuple(y_infer.shape))
            )
